...
export class AdminReviewApplicationComponent implements OnInit, OnDestroy {
  ...
  getJobData() {
    this.rs.get("/forms/job/view/1").subscribe((formData) => {
      ...
      this.rs.get("/forms/application/view/1/" + this.username).subscribe(
        (applicationData) => {
          this.gApp = applicationData.application;
          if (
            applicationData.status == "Application not found" ||
            this.gApp.answers.length === 0
          ) {
    ...
    this.rs.get("/forms/job/view/" + this.formID).subscribe((formData) => {
      ...
      // GET request to retrieve previous application answers
      this.rs
        .get("/forms/application/view/" + this.formID + "/" + this.username)
        .subscribe(
          (applicationData) => {
  ...
  updateStatus() {
    ...
    this.rs
      .post("/forms/application/status", postData, null, "urlencoded")
      .subscribe(
        (data) => {
