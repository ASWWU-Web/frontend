...
export class AdminBallotsComponent implements OnInit {
  ...
  onCountVotes(election: number): void {
    const countUrl = this.ers.readElectionCount(
      this.electionsData[election].id,
    );
    let voteMessage = "";
    countUrl.subscribe((data) => {
      ...
      for (const position of data.positions) {
        ...
        for (const candidate of position.votes) {
          voteMessage +=
            "  " +
            this.prettyUsername(candidate.candidate) +
            " - " +
            candidate.votes +
            " votes\n";
        }
  ...
  onSelectElection(election: number): void {
    ...
    const candidateObservable = this.ers.listCandidates(
      this.electionsData[election].id,
    );
    ...
    this.availablePositions = this.positionsData.filter(
      (position) =>
        position.election_type === this.selectedElection.election_type,
    );
  }
  ...
  onSaveBallot(ballot: BallotPOST) {
    ...
    ballotObservable.subscribe(
      ...
      (error) => {
        console.log(error);
        window.alert(
          `This ballot could not be saved completely. ` +
            ...
            `\n` +
            `Voting for: ` +
            this.prettyUsername(ballot.vote) +
            `\n` +
            `Position: ` +
            this.idToPosition(ballot.position),
        );
      },
    );
  }
  ...
  deleteBallot(index: number): void {
    ...
    const ballotObservable = this.ers.removeBallot(
      this.selectedElection.id,
      this.ballots[index].id,
    );
