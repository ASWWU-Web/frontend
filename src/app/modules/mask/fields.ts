...
export const FieldsInOrder: string[] = FieldSections.reduce((a, b) => {
  return a.concat(b);
}).filter((a) => {
  return PrivateFields.indexOf(a) < 0;
});
export const FieldsForUpdating: string[] = FieldSections.reduce((a, b) => {
  return a.concat(b);
});
export const FieldsForSearching: string[] = FieldSections.reduce((a, b) => {
  return a.concat(b);
}).filter((a) => {
  return a !== 'privacy';
});
