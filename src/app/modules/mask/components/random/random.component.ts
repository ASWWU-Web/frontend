  ...
  template: `
    <div class="container">
      ...
      <div style="margin-top: 40px">
        ...
        <h1
          *ngIf="isLoading"
          style="text-transform: capitalize !important; color: white; text-align: center"
        >
          LOADING
        </h1>
      </div>
...
export class RandomComponent implements OnInit {
  ...
  getRandom(): any {
    ...
    while (
      newProfile["photo"] === "images/mask_unknown.png" ||
      newProfile["photo"] === "None" ||
      !newProfile["photo"]
    ) {
      random = Math.floor(Math.random() * this.allProfiles.length);
      newProfile = this.allProfiles[random];
    }
    const profileObservable = this.mrs.readProfile(
      CURRENT_YEAR,
      newProfile["username"],
    );
