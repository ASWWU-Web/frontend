...
export class RequestService {
  ...
  * */
  private verifyGet(uri: string, afterRequest, catchError): void {
    ...
    this.http.get(req.url, req.options).pipe(
        map(res => res.json()))
        .subscribe(
  ...
  * */
  ...
  post(uri: string, data: any, afterRequest, catchError): void {
    ...
    this.verify();
    const req = this.createRequest(uri);
    this.http.post(req.url, body, req.options).pipe(
      map(res => res.json()))
      .subscribe(
        data => afterRequest(data),
        err => (catchError ? catchError(err) : console.error(err))
  ...
  postxwww(uri: string, data: any, afterRequest, catchError): void {
    ...
    const body = this.objToSearchParams(data);
    this.verify();
    const req = this.createRequest(uri, 'application/x-www-form-urlencoded');
    this.http.post(req.url, body.toString(), req.options).pipe(
      map(res => res.json()))
      .subscribe(
        data => afterRequest(data),
        err => (catchError ? catchError(err) : console.error(err))
