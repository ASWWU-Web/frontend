...
export class AdminReviewApplicationComponent {
    ...
    RS_OLD.verify((user) => {
      if(user) {
        ...
        rs.get('/forms/job/view/'+this.formID).subscribe((data) => {
          ...
          //GET request to retrieve previous application answers
          RS_OLD.get('/forms/application/view/'+this.formID+'/'+this.username, (data) => {
  ...
  updateStatus() {
    this.RS_OLD.postxwww('/forms/application/status', {jobID: this.formID, username: this.username, status: this.app.status},
      (data)=>{
